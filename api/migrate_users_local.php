...
if (!$conn) {
    ...
    echo json_encode(['status' => 'error', 'message' => 'Erro na conexão: ' . mysqli_connect_error(), 'data' => null]);
    exit;
}
...
elseif ($action === 'get_status') {
    ...
    if (!$result_clients || !$result_photographers || !$result_clients_migrated || !$result_photographers_migrated) {
        response('error', 'Erro ao verificar status: ' . $conn->error);
    }
    ...
    response('success', 'Status da migração', [
        'clientes' => [
            'total' => (int)$data_clients['total_clients'],
            'migrados' => (int)$data_clients_migrated['migrated_clients'],
            'pendentes' => (int)$clients_to_migrate
        ],
        'fotografos' => [
            'total' => (int)$data_photographers['total_photographers'],
            'migrados' => (int)$data_photographers_migrated['migrated_photographers'],
            'pendentes' => (int)$photographers_to_migrate
        ]
    ]);
}
